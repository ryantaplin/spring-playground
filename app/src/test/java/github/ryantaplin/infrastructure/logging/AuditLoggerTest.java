...
class AuditLoggerTest {
    ...
    class Infrastructure {
        ...
        void loggerNameIsApplication() {
            assertThat(logger.getName()).isEqualTo("AUDIT");
        }
    }
    ...
    class Format {
        ...
        void generalLogFormat(CapturedOutput output) {
            ...
            assertThat(output.getAll()).matches(pattern(escape("""
                    [START] [CONTEXT=AUDIT] [LEVEL=INFO] [AT=\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2},\\d{3}] [Test worker] (github.ryantaplin.infrastructure.logging.AbstractDelegatingLogger: 146) [TraceId=random-uuid]
                    some message
                    [END]
                    """)));
        }
